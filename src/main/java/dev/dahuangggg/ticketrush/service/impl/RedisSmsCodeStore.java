...
public class RedisSmsCodeStore implements SmsCodeStore {
    ...
    public void save(String phone, String code) {
        Boolean notOnCooldown = redisTemplate.opsForValue()
                .setIfAbsent(RedisKeyRegistry.smsCooldownKey(phone), "1", COOLDOWN_DURATION);
        if (!Boolean.TRUE.equals(notOnCooldown)) {
            throw new SmsCooldownException();
        }
        redisTemplate.opsForValue().set(RedisKeyRegistry.smsCodeKey(phone), code, codeTtl);
    }
    ...
    public boolean matches(String phone, String code) {
        String storedCode = redisTemplate.opsForValue().get(RedisKeyRegistry.smsCodeKey(phone));
        return code.equals(storedCode);
    }

    @Override
    public void delete(String phone) {
        redisTemplate.delete(RedisKeyRegistry.smsCodeKey(phone));
    }
}
